...
1. Planner（router LLM，非流式，response_format json）：把用户问题拆成 2~5 个子任务，每个指定专家 agent + 子问题；编排层强制保留 deep_researcher。预测类任务按“资料型专家 → deep_researcher 建图 → predictor 研究交接”排序。发 `agent_step {phase:"plan", plan}`。
