...
- **实验性多智能体情景推演**：从证据图异步生成参与方行动与未来情景分支，支持动态
  角色预算、运行前预览、进度恢复和安全取消；coverage-balanced compiler 的 20 例扩大
  留出验证把有效决策角色覆盖率从 76.7% 提升到 100%，遗漏角色从 31 个降到 0；当前仅
  输出未校准情景。
...
| 👥 深度研究团队 | 多维度深度问题 | Planner 拆 2~5 子任务 → 最多 5 位专家串行 → 证据图 → 综合与复核 |
